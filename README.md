...
### Lookup Your Financial Institution
  ...
  `net.sf.ofx4j.client.FinancialInstitutionDataStore` to list the available financial institution data.
  Alternatively, use `net.sf.ofx4j.client.impl.BaseFinancialInstitutionData` to construct your own data if your
  FI data isn't available.

  The two default implementations of `FinancialInstitutionDataStore` are `net.sf.ofx4j.client.impl.LocalResourceFIDataStore`
  and `net.sf.ofx4j.client.impl.OFXHomeFIDataStore`. The former looks up the data store list from a local resource (default is the
...
You can use OFX4J to implement your own OFX server. To do so, you must implement the `net.sf.ofx4j.server.OFXServer`
interface.  Once an adequate implementation of this interface is exists, you can use the `net.sf.ofx4j.server.OFXServlet`
...
`net.sf.ofx4j.io.OFXReader`. We suggest the `net.sf.ofx4j.io.nanoxml.NanoXMLOFXReader`, which uses utilities provided by the
...
Or you want to unmarshal the OFX directly to a Java object, use the `net.sf.ofx4j.io.AggregateUnmarshaller`.

### Writing OFX data

You can also write OFX data using an instance of `net.sf.ofx4j.io.OFXWriter`. `net.sf.ofx4j.io.v1.OFXV1Writer` is used to write OFX version 1 and
`net.sf.ofx4j.io.v2.OFXV2Writer` is used to write OFX version 2. If you want to marshal from Java objects, use the
`net.sf.ofx4j.io.AggregateMarshaller`
